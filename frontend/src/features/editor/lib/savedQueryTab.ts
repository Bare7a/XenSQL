...
export function findTabForSavedQuery(tabs: EditorTab[], saved: SavedQuery): EditorTab | undefined {
  const linked = tabs.find((t) => t.savedQueryId === saved.id);
  if (linked) return linked;

  return tabs.find(
    (t) => !t.savedQueryId && t.title === saved.name && (!saved.connectionId || t.connectionId === saved.connectionId),
  );
}
