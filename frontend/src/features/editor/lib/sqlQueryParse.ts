...
const ALIAS_SRC =
  '(?:"[^"]+")|(?:\'[^\']+\')|(?:`[^`]+`)|' +
  `(?:(?!(?:${[...ALIAS_STOP_WORDS].join('|')})\\b)[a-zA-Z_][\\w]*)`;
...
const FROM_TABLE_LIST_RE = new RegExp(
  `\\bFROM\\s+(${FROM_ITEM_SRC}(?:\\s*,\\s*${FROM_ITEM_SRC})*)`,
  'gi'
);
const FROM_ITEM_RE = new RegExp(
  `(${IDENT_SRC})(?:\\s*\\.\\s*(${IDENT_SRC}))?(?:\\s+(?:AS\\s+)?(${ALIAS_SRC}))?`,
  'gi'
);
...
export function resolveTableName(
  ...
  const match =
    tables.find(
      (t) => t.name.toLowerCase() === lcName && (!lcHint || t.schema.toLowerCase() === lcHint)
...
export function parseQueryContext(
  ...
  while ((m = TABLE_BINDING_RE.exec(sql)) !== null) {
    if (!m[1]) continue;
    ...
    const alias =
      aliasUnquoted && !ALIAS_STOP_WORDS.has(aliasUnquoted.toLowerCase())
        ? aliasUnquoted
        : undefined;
  ...
  while ((m = UPDATE_BINDING_RE.exec(sql)) !== null) {
    if (!m[1]) continue;
    const binding = parseTableRef(m[1], m[2], tables, schemas, driver);
  ...
  while ((m = FROM_TABLE_LIST_RE.exec(sql)) !== null) {
    const list = m[1];
    const listIndex = m.index;
    FROM_ITEM_RE.lastIndex = 0;
    let im: RegExpExecArray | null;
    while ((im = FROM_ITEM_RE.exec(list)) !== null) {
      if (!im[1]) continue;
      const binding = parseTableRef(im[1], im[2], tables, schemas, driver);
      const aliasRaw = im[3];
      const aliasUnquoted = aliasRaw ? unquoteIdent(aliasRaw) : undefined;
      const alias =
        aliasUnquoted && !ALIAS_STOP_WORDS.has(aliasUnquoted.toLowerCase()) ? aliasUnquoted : undefined;
...
export function resolveDotCompletion(
  ...
  if (dot.segments.length === 2) {
    return resolveTableName(
      unquoteIdent(dot.segments[1]),
      unquoteIdent(dot.segments[0]),
      tables,
      schemas,
      driver
    );
  }
  return resolveQualifierToTable(
    unquoteIdent(dot.segments[0]),
    bindings,
    tables,
    schemas,
